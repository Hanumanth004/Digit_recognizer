...
def lossFunc(X_tmp):
    ...
    for i in xrange(len(X_tmp)):
        ...
        if(i%BATCH_SIZE==0):
            mem=dWh*dWh
            Wh+=-learning_rate*dWh / np.sqrt(mem + 1e-8)
            
            mem=dWo*dWo
            Wo+=-learning_rate*dWo / np.sqrt(mem + 1e-8)

            mem=dbh*dbh
            bh+=-learning_rate*dbh / np.sqrt(mem + 1e-8)

            mem=dbo*dbo
            bo+=-learning_rate*dbo / np.sqrt(mem + 1e-8)
...
"""
X_tmp=X_tr
f2=open("./log_data/sig_adagrad.dat",'w+')
for ep in xrange(50):
    loss=lossFunc(X_tmp)
